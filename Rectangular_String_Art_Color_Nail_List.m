...
% Click run
% Note this will save a four .txt files which contains the nail list orders

%% User Inputs:
filename = 'Euler.png'
Num_Nails = 400;                % Enter the number of nails around the Canvas (must be even number)
...
% Cyan
[Plot_Lines_x_c, Plot_Lines_y_c, Lines_list_c] = StringArtCalc(f_c,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,spacing,b,h);
% Magenta
[Plot_Lines_x_m, Plot_Lines_y_m, Lines_list_m] = StringArtCalc(f_m,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,spacing,b,h);
% Yellow
[Plot_Lines_x_y, Plot_Lines_y_y, Lines_list_y] = StringArtCalc(f_y,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,spacing,b,h);
% Black
[Plot_Lines_x_k, Plot_Lines_y_k, Lines_list_k] = StringArtCalc(f_k,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,spacing,b,h);
...
%% Write .txt file
% Cyan
WriteTextFile('Cyan', filename, Num_Nails, N_h, N_b, ratio, Lines_list_c)
% Magenta
WriteTextFile('Magenta', filename, Num_Nails, N_h, N_b, ratio, Lines_list_m)
% Yellow
WriteTextFile('Yellow', filename, Num_Nails, N_h, N_b, ratio, Lines_list_y)
% Black
WriteTextFile('Black', filename, Num_Nails, N_h, N_b, ratio, Lines_list_k)
...
function WriteTextFile(Color, filename, Num_Nails, N_h, N_b, r, Lines_list)
    ...
    outFileName = sprintf('RectangularColorStringArtNailList_%s_%s_%d.txt', ...
                          Color, imageFileNameWithoutExt, Num_Nails);
    ...
    % Preamble
    fprintf(fid, 'Color of string is: Color = %s\n', Color);
    ...
    fprintf(fid, 'Label these nails 1 to %d. First place down Cyan (C) string, then Magenta (M)\n', Num_Nails);
    fprintf(fid, 'string, then Yellow (Y) string, then Black (K) string.\n\n');
    
    % Heading before the nail list
    fprintf(fid, 'The following nail list is for %s string:\n', Color);
