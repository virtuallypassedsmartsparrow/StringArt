...
A = imresize(A,[ideal_image_size, ideal_image_size]);
A = double(A) / 255;
% BW = 1-double(BW)/255;
% f = BW;
% A = double(BW)/255;
...
% Cyan
[Plot_Lines_x_c, Plot_Lines_y_c, Lines_list_c] = StringArtCalc(f_c,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,R);
% Magenta
[Plot_Lines_x_m, Plot_Lines_y_m, Lines_list_m] = StringArtCalc(f_m,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,R);
% Yellow
[Plot_Lines_x_y, Plot_Lines_y_y, Lines_list_y] = StringArtCalc(f_y,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,R);
% Black
[Plot_Lines_x_k, Plot_Lines_y_k, Lines_list_k] = StringArtCalc(f_k,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,R);
...
%% Write .txt file
% Cyan
WriteTextFile('Cyan', filename, Num_Nails, Lines_list_c)
% Magenta
WriteTextFile('Magenta', filename, Num_Nails, Lines_list_m)
% Yellow
WriteTextFile('Yellow', filename, Num_Nails,Lines_list_y)
% Black
WriteTextFile('Black', filename, Num_Nails, Lines_list_k)
...
function [Plot_Lines_x, Plot_Lines_y,Lines_list] = StringArtCalc(f,ideal_image_size,Num_Nails,d,p_min,tstart,tend,p_theshold,num_max_lines,R)
    ...
    for i = 1:length(psi_1_number)
        if mod(i, 2) == 0 % even
            Lines_list(i) = psi_1_number(i);
        else
            Lines_list(i) = psi_2_number(i);
        end
    end
...
function WriteTextFile(Color,filename, Num_Nails, Lines_List)
    ...
    outFileName = sprintf('CircleStringArtNailList_%s_%s_%d.txt', ...
    ...
    fprintf(fid, ...
        'Order of colored string:Cyan(C)->Magenta(M)->Yellow(Y)->Black(K) \n\n');
